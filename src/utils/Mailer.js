...
const sendVerifyMailToken = (token, email, name) => {
    const emailBody = {
        body: {
            ...
            intro: `Welcome <b style="text-transform: capitalize;">${name}</b>, glad to have you onboard. Please verify your mail to enjoy premium access.`,
            action: {
                instructions:
      ...
      \n\n<a href="${url}/auth/activate_user?email=${email}&token=${token}">${url}/auth/activate_user?email=${email}&token=${token}</a>`
        }
    };
